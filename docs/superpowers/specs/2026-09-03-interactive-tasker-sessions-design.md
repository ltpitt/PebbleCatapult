...
- **Tasker plugin action** starts a session with a request type, title, and
  payload, waits asynchronously, and exposes result variables including
  `%catapult_result_id`, `%catapult_result_value`, and `%catapult_status`.
...
- **Watch UI** provides reusable list and confirmation windows. It reports
  selection, cancellation, or errors without knowing about Tasker.
...
Only one interactive session is active at a time. Session IDs make stale and
duplicate responses harmless.
